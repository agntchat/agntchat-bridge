...
@pytest.mark.asyncio
async def test_returned_reply_uses_latest_seen_anchor_and_drops_on_409(executor):
    """The server settles a stale draft itself (posting it when it still adds
    something), so a 409 always means drop: sent once, never re-posted, the
    message still acked and no notice posted."""
    @executor.on_message
    ...
    stale = StaleContextError("stale", new_messages=[])
